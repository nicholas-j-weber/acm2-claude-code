...
| 4 | Companion window + bridge | **Prototyped & committed.** `companion/server.mjs` (built-ins only) + `companion/index.html`, `/api/pending` and `/api/resolve`, launch skill at `.claude/skills/context-window`. Smoke-tested directly (curl + a real pending file: listed, edited, resolved, moved) — port `4317`. Untested via the actual `/context-window` launch flow (nohup+disown+browser) and untested in a real browser. | — | n/a — done |
