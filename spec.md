...
| 3.2 | Agent-shared memory w/ provenance | **Prototyped & committed.** `CLAUDE.md` instructs the main loop to select relevant active entries and embed them in a labeled section of the spawn prompt, omitted entirely when nothing's relevant; explicitly read-only. Reuses the Agent tool as-is — no new code. Untested live (needs a session that actually spawns a subagent with relevant memory present). | — | n/a — done |
