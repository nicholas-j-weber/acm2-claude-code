...
| 3.4 | Agent-driven relevance pruning | **Prototyped & committed.** `CLAUDE.md` instructs the assistant to propose pruning via `Edit` on frontmatter `status`, one at a time, `Bash`-toggle excluded as a silent bypass. Reuses §3.1's hook as-is — no new code. Untested live (needs a session with an actually-irrelevant active memory to trigger it). | — | n/a — done |
