...
| 4 | Companion window: per-memory management UI | **Built, committed, and verified live.** `companion/index.html`/`server.mjs` extended beyond the pending/resolved bridge to list, edit, pin/unpin, activate/deactivate, delete, and compress-into-summary any `memory/*.md` entry directly. Driven end-to-end against the running server (not just curled): edit persisted a body change, pin set `pinned: true`, delete removed a file, compress created a summary entry with `summarizes:` and flipped the summarized entry to `status: inactive` — all confirmed both via the API responses and a real browser screenshot of the rendered cards. | — | n/a — done |
| — | Native auto-memory disabled | **Verified live via A/B test.** `autoMemoryEnabled: false` in `.claude/settings.json`. Without it, a real `claude -p` session wrote an unprompted fact straight into `~/.claude/projects/<project>/memory/`, no approval, outside this spec's governance entirely. With the flag restored, the same-shaped prompt produced no file there. This was the concern that motivated the (since-abandoned) Agent SDK rebuild; the flag closes it without any of that. | — | n/a — done |
| — | `memory-history.mjs` | **Prototyped & committed.** Thin wrapper over `git log -p --follow -- memory/<file>`, since §3.6 already resolved git history as the Version record — this just saves a developer from memorizing the flags. Not live-tested against a browser (it's a CLI script), smoke-tested against a real tracked file. | — | n/a — done |

**Reading this table:** every piece is done. §3.1, §3.2, §3.4, §3.7, §3.10,
§4 (both the original bridge and the per-memory management UI), and the
auto-memory-disable fix are verified live (§3.7's auto-block half stays
`--self-test`-only, permanently — Claude Code's own context-pressure trigger
can't be forced on demand); §3.3, §3.5, §3.9, and `memory-history.mjs` are
prototyped and committed but not live-tested against a browser (not
applicable to the latter, being a CLI script). §3.6 is resolved with nothing
left to build. Every open design question in §5 is resolved. Nothing in this
spec is left unbuilt.
